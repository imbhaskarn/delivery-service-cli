...
program
    ...
    if(options.args.length > 0){
      ...
      return;
		...
		const packageString = new Package(str.price, str.items, str.packages, 'cost');
  ...
  program
	...
	.requiredOption("-p, --price <char>, base delivery cost of a package")
	.requiredOption("-n, --items <number>, No. of package for cost calculation")
  .requiredOption("-vi, --vehicle <number...>", "vehicle info like speed numbers and capacity")
	.option(
		"-ps, --packages <packages...>",
		"Format: [pkgid, pkgweight, distance-from-hub, offer-code]"
	)
	.action((str, options) => {
    console.log(str)
		const packageString = new Package(str.price, str.items, str.packages, 'time', str.vehicle);
		const packageList = packageString.parsePackage();
		const validation = packageString.validatePackage(packageList, str.vehicle);
    if(validation.error){
      console.log(chalk.red( validation.errors))
      return 0
    }
